      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button @click="$emit('view-json', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="document" size="sm" class="text-sky-500" />
              {{ t('admin.accounts.viewJson') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'view-json', 'schedule', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy'])
const { t } = useI18n()
