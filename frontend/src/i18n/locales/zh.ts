export default {
  ...
  admin: {
    ...
    modelStatus: {
      title: '模型状态',
      description: '监控账号模型可用性与延迟',
      createTarget: '新增探测目标',
      editTarget: '编辑探测目标',
      formHint: '选择现有账号和模型，系统会按间隔自动探测并记录最新状态。',
      targets: '探测目标',
      targetsHint: '失败项会自动排在前面，便于优先处理。',
      account: '账号',
      selectAccount: '请选择账号',
      model: '模型',
      modelPlaceholder: '例如 gpt-4.1-mini',
      interval: '探测间隔（秒）',
      intervalValue: '每 {seconds} 秒执行一次',
      timeout: '超时（秒）',
      enabled: '启用探测',
      latency: '延迟',
      lastChecked: '最近检测',
      ...
      runNow: '立即检测',
      history: '最近历史',
      historyHint: '查看最近 20 次探测结果。',
      historyTitle: '{name} 的最近历史',
      selectTargetForHistory: '点击上方某个目标的“最近历史”即可查看探测详情。',
      noHistory: '暂无历史记录',
      noSuccessYet: '暂无成功记录',
      lastSuccessAt: '最近成功：{time}',
      failures: '连续失败 {count} 次',
      namePlaceholder: '留空将自动生成名称',
      showDisabled: '显示已禁用目标',
      emptyTitle: '还没有探测目标',
      emptyDescription: '先创建一个目标，系统会自动周期性检测模型状态。',
      deleteConfirm: '确定要删除这个探测目标吗？',
      created: '探测目标已创建',
      updated: '探测目标已更新',
      deleted: '探测目标已删除',
      runTriggered: '已执行一次即时探测',
      failedToLoad: '加载模型状态失败',
      failedToSave: '保存模型状态失败',
      failedToDelete: '删除模型状态失败',
      failedToRun: '执行即时探测失败',
      failedToLoadHistory: '加载探测历史失败',
