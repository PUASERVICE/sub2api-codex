...
wait_for_model_status_and_bootstrap() {
...
start_model_status_sidecar() {
    ...
    MODEL_STATUS_BLOCKING_START="${MODEL_STATUS_BLOCKING_START:-true}"
    ...
    if [ "$MODEL_STATUS_BLOCKING_START" = "true" ]; then
        wait_for_model_status_and_bootstrap
    else
        (
            wait_for_model_status_and_bootstrap
        ) >>/tmp/model-status.log 2>&1 &
        echo "[model-status] non-blocking startup enabled; bootstrap running in background"
    fi
}
